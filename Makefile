...
bin/volume-modifier-for-k8s: | bin
	CGO_ENABLED=1 GOEXPERIMENT=boringcrypto GOOS=linux GOARCH=$(ARCH) go build -mod=mod -ldflags ${LDFLAGS} -tags fips -o bin/volume-modifier-for-k8s ./cmd && go tool nm $@ | grep 'sig\.FIPSOnly'
